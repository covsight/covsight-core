...
# under the License.
"""UCIS-XML reader -- builds a MemUCIS model from a UCIS-XML document.

Ported from pyucis (`ucis/xml/xml_reader.py`, Apache-2.0, ASF-licensed) when
UCIS-XML was promoted to a first-class citizen of covsight-core rather than a
`covsight-xml` plugin. The body is deliberately kept close to the original so
the two can still be diffed; the substantive changes are:

  - lxml -> stdlib `xml.etree.ElementTree`, so core gains no dependency. lxml
    was used in exactly two places: parsing, and the namespace-stripping walk.
  - pyucis exported flat `UCIS_*` constants from its top-level package; core
    exposes the same values as enum members. They are re-aliased below so the
    body needs no edit.
"""
...
from covsight.core.api.enums import (
    CoverTypeT, FlagsT, ScopeTypeT, SourceT,
)
...
from covsight.core.xml.validator import validate_ucis_xml

# pyucis exported these as flat module constants; core keeps the same values on
# the enums. Aliasing here rather than rewriting ~800 lines of body keeps this
# file diffable against its origin.
...
#        self.db.setWrittenTime(self.getAttrDateTime(root, "writtenTime"))
    ...
    def readHistoryNode(self, histN):
        ...
        from covsight.core.api.enums import HistoryNodeKind
    ...
    def getAttrDateTime(self, e, name):
        """Convert an xsd:dateTime attribute to the YYYYMMDDHHMMSS form the model uses.

        Diverges from pyucis deliberately. xsd:dateTime permits an optional
        timezone designator -- 'Z' or +/-HH:MM -- and pyucis accepted neither,
        so any document written by ucis_writer (which emits UTC as 'Z') failed
        to read back. This was the first thing the C-writer/Python-reader
        cross-check caught.

        An offset-aware value is normalised to UTC so documents written in
        different timezones stay comparable; a naive value is taken as-is,
        which is what pyucis did.
        """
        val = e.get(name)
        if val is None:
            return None
        ...
        try:
            ...
            for fmt in ("%Y-%m-%dT%H:%M:%S.%f", "%Y-%m-%dT%H:%M:%S"):
                try:
                    ...
                    continue
        if dt is None:
            raise ValueError(f"Cannot parse xsd:dateTime: {val!r}")
