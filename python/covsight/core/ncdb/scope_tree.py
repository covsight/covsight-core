...
class ScopeTreeReader:
    ...
    def _read_toggle_pair(self, data: bytes, offset: int, parent, counts_iter):
        ...
        togglebin_at_least = COVER_TYPE_DEFAULTS.get(CoverTypeT.TOGGLEBIN, (0, 0, 1))[1]
        for (bin_name, count) in ((TOGGLE_BIN_0_TO_1, count_0to1),
            ...
            cd.data = count
            cd.at_least = togglebin_at_least
    ...
    def _read_regular_scope(self, data: bytes, offset: int, parent, counts_iter):
        ...
        if scope_type == ScopeTypeT.INSTANCE:
            ...
            if du_scope is None:
                ...
                from covsight.core.mem.mem_scope import MemScope as _MemScope
                du_scope = _MemScope(
        ...
        for _ in range(num_coveritems):
            ...
            cd.data = count
            cd.at_least = at_least  # always set from the decoded/defaulted value
