...
class TestApiCrossCoverage:
    ...
    def test_create_cross(self, backend):
        ...
        inst = db.createInstance("inst", None, 1, SourceT.VLOG, ScopeTypeT.INSTANCE, 
                                du, FlagsT.INST_ONCE)
        
        # Create covergroup with coverpoints
        cg = inst.createCovergroup("cg_cross", SourceInfo(file_h, 5, 0), 1, SourceT.VLOG)
        
        # Create two coverpoints that will be crossed
        cp1 = cg.createCoverpoint("cp1", SourceInfo(file_h, 6, 0), 1, SourceT.VLOG)
        # XML requires bins in coverpoints
        if backend_name == "xml":
            cp1.createBin("cp1_bin", SourceInfo(file_h, 6, 0), 1, 0, "val1", CoverTypeT.CVGBIN)
        
        cp2 = cg.createCoverpoint("cp2", SourceInfo(file_h, 7, 0), 1, SourceT.VLOG)
        # XML requires bins in coverpoints
        if backend_name == "xml":
            cp2.createBin("cp2_bin", SourceInfo(file_h, 7, 0), 1, 0, "val2", CoverTypeT.CVGBIN)
        
        # Create cross coverage
        cross = cg.createCross("cross_cp1_cp2", SourceInfo(file_h, 8, 0), 
                              1, SourceT.VLOG, [cp1, cp2])
    ...
    def test_cross_with_bins(self, backend):
        ...
        du = db.createScope("work.m", SourceInfo(file_h, 1, 0),
                           1, SourceT.VLOG, ScopeTypeT.DU_MODULE,
                           FlagsT.SCOPE_UNDER_DU | FlagsT.INST_ONCE)
        inst = db.createInstance("i", None, 1, SourceT.VLOG, ScopeTypeT.INSTANCE, 
                                du, FlagsT.INST_ONCE)
        
        # Create covergroup with coverpoints and bins
        cg = inst.createCovergroup("cg", SourceInfo(file_h, 5, 0), 1, SourceT.VLOG)
        ...
        cp2 = cg.createCoverpoint("cp2", SourceInfo(file_h, 9, 0), 1, SourceT.VLOG)
    ...
    def test_cross_three_coverpoints(self, backend):
        ...
        cg = inst.createCovergroup("cg_3way", SourceInfo(file_h, 5, 0), 1, SourceT.VLOG)
        
        cp1 = cg.createCoverpoint("addr", SourceInfo(file_h, 6, 0), 1, SourceT.VLOG)
        # XML requires bins in coverpoints
        if backend_name == "xml":
            cp1.createBin("addr_bin", SourceInfo(file_h, 6, 0), 1, 0, "val1", CoverTypeT.CVGBIN)
        
        cp2 = cg.createCoverpoint("data", SourceInfo(file_h, 7, 0), 1, SourceT.VLOG)
        if backend_name == "xml":
            cp2.createBin("data_bin", SourceInfo(file_h, 7, 0), 1, 0, "val2", CoverTypeT.CVGBIN)
        
        cp3 = cg.createCoverpoint("ctrl", SourceInfo(file_h, 8, 0), 1, SourceT.VLOG)
        if backend_name == "xml":
            cp3.createBin("ctrl_bin", SourceInfo(file_h, 8, 0), 1, 0, "val3", CoverTypeT.CVGBIN)
        
        # Create 3-way cross
        cross = cg.createCross("addr_data_ctrl", SourceInfo(file_h, 9, 0), 
                              1, SourceT.VLOG, [cp1, cp2, cp3])
    ...
    def test_multiple_crosses(self, backend):
        ...
        cg = inst.createCovergroup("cg_multi", SourceInfo(file_h, 5, 0), 1, SourceT.VLOG)
        
        cp_a = cg.createCoverpoint("a", SourceInfo(file_h, 6, 0), 1, SourceT.VLOG)
        # XML requires bins in coverpoints
        if backend_name != "xml":
        ...
        cp_b = cg.createCoverpoint("b", SourceInfo(file_h, 7, 0), 1, SourceT.VLOG)
        if backend_name == "xml":
            cp_b.createBin("b_bin", SourceInfo(file_h, 7, 0), 1, 0, "b_val", CoverTypeT.CVGBIN)
        
        cp_c = cg.createCoverpoint("c", SourceInfo(file_h, 8, 0), 1, SourceT.VLOG)
        if backend_name == "xml":
