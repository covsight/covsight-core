...
class TestApiCoverItems:
    ...
    def _make_cg_cp(self, db):
        ...
        inst = db.createInstance("i_m1", None, 1, SourceT.VLOG,
    ...
    def test_get_cover_item_name(self, backend):
        ...
        cd = CoverData(CoverTypeT.CVGBIN, 5)
    ...
    def test_set_cover_data(self, backend):
        ...
        for item in items:
            if item.getName() == "bin_set":
                new_data = CoverData(CoverTypeT.CVGBIN, 0)
                new_data.data = 99
    ...
    def test_multiple_cover_items(self, backend):
        ...
        for i in range(5):
            cd = CoverData(CoverTypeT.CVGBIN, i * 2)
