...
class TestApiFileHandles:
    ...
    def test_file_handle_in_covergroup(self, backend):
        ...
        inst = db.createInstance("i_top", None, 1, SourceT.VLOG, 
                                ScopeTypeT.INSTANCE, du, FlagsT.INST_ONCE)
        
        # Create covergroup with source info
        cg = inst.createCovergroup("cg1", SourceInfo(file_cov, 50, 0), 1, SourceT.VLOG)
        
        # Create coverpoint with source info
        cp = cg.createCoverpoint("cp1", SourceInfo(file_cov, 55, 0), 1, SourceT.VLOG)
