...
class TestApiFSMCoverage:
    ...
    def test_create_fsm_scope(self, backend):
        ...
        fsm = inst.createScope("state_machine", None, 1, SourceT.VLOG,
                               ScopeTypeT.FSM, FlagsT.INST_ONCE)
    ...
    def test_fsm_create_states(self, backend):
        ...
        db = create_db()
        inst = self._make_inst(db)
        fsm = inst.createScope("fsm1", None, 1, SourceT.VLOG,
                               ScopeTypeT.FSM, FlagsT.INST_ONCE)
    ...
    def test_fsm_scope_in_hierarchy(self, backend):
        ...
        db = create_db()
        inst = self._make_inst(db)
        inst.createScope("fsm_ctrl", None, 1, SourceT.VLOG,
                         ScopeTypeT.FSM, FlagsT.INST_ONCE)
        inst.createScope("fsm_data", None, 1, SourceT.VLOG,
                         ScopeTypeT.FSM, FlagsT.INST_ONCE)
