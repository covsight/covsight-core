...
class TestApiCoverpoints:
    ...
    def test_create_bins(self, backend):
        ...
        inst = db.createInstance("i", None, 1, SourceT.VLOG, ScopeTypeT.INSTANCE, 
                                du, FlagsT.INST_ONCE)
        cg = inst.createCovergroup("cg", SourceInfo(file_h, 5, 0), 1, SourceT.VLOG)
        cp = cg.createCoverpoint("cp", SourceInfo(file_h, 10, 0), 1, SourceT.VLOG)
    ...
    def test_bin_counts(self, backend):
        ...
        for bin_name, count in test_counts.items():
            cp.createBin(bin_name, SourceInfo(file_h, 11, 0), 1, count, "x", CoverTypeT.CVGBIN)
    ...
    def test_coverpoint_atleast(self, backend):
        ...
        cp.createBin("below", SourceInfo(file_h, 11, 0), 10, 5, "0", CoverTypeT.CVGBIN)
        cp.createBin("at", SourceInfo(file_h, 12, 0), 10, 10, "1", CoverTypeT.CVGBIN)
        cp.createBin("above", SourceInfo(file_h, 13, 0), 10, 20, "2", CoverTypeT.CVGBIN)
    ...
    def test_bin_names(self, backend):
        ...
        for name in special_names:
            cp.createBin(name, SourceInfo(file_h, 11, 0), 1, 5, "x", CoverTypeT.CVGBIN)
    ...
    def test_multiple_coverpoints(self, backend):
        ...
        for i in range(3):
            cp_name = f"cp_{i}"
            cp = cg.createCoverpoint(cp_name, SourceInfo(file_h, 10+i, 0), 1, SourceT.VLOG)
            # Add at least one bin to each coverpoint
            cp.createBin(f"bin_{i}", SourceInfo(file_h, 20+i, 0), 1, 5, "x", CoverTypeT.CVGBIN)
    ...
    def test_coverpoint_weight(self, backend):
        ...
        cp1 = cg.createCoverpoint("cp_w1", SourceInfo(file_h, 10, 0), 1, SourceT.VLOG)
        cp1.createBin("b1", SourceInfo(file_h, 11, 0), 1, 5, "x", CoverTypeT.CVGBIN)
        
        cp2 = cg.createCoverpoint("cp_w10", SourceInfo(file_h, 12, 0), 10, SourceT.VLOG)
        cp2.createBin("b2", SourceInfo(file_h, 13, 0), 1, 5, "x", CoverTypeT.CVGBIN)
        
        cp3 = cg.createCoverpoint("cp_w100", SourceInfo(file_h, 14, 0), 100, SourceT.VLOG)
        cp3.createBin("b3", SourceInfo(file_h, 15, 0), 1, 5, "x", CoverTypeT.CVGBIN)
